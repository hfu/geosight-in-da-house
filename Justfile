...
install:
    ...
    # Detect architecture and create platform-specific override if needed
    ARCH=$(uname -m)
    if [[ "$ARCH" == "aarch64" || "$ARCH" == "arm64" ]]; then
        echo "🔧 Detected ARM64 architecture - adding platform override for db service..."
        
        # Create a platform-specific override file
        {
            echo "# Platform-specific override for ARM64 architecture"
            echo "# Generated by geosight-in-da-house for Raspberry Pi compatibility"
            echo "version: '3.9'"
            echo ""
            echo "services:"
            echo "  db:"
            echo "    platform: linux/arm64"
            echo "    # Use the same image but explicitly specify platform"
            echo "    # Docker will pull ARM64 variant or use emulation"
        } > deployment/docker-compose.override.arm64.yml
