...
install:
    ...
    if [[ "$ARCH" == "aarch64" || "$ARCH" == "arm64" ]]; then
        echo "🔧 Detected ARM64 architecture - setting up custom builds for compatibility..."
...
run: _check-docker _check-geosight
    ...
    if [[ "$ARCH" == "aarch64" || "$ARCH" == "arm64" ]] && [ -f deployment/docker-compose.override.arm64.yml ]; then
        echo "🏗️  Building ARM64-compatible images..."
...
cleanup_partial_initialization() {
    ...
    cleanup_statements=(
        ...
        "DROP TABLE IF EXISTS geosight_data_indicatorrule CASCADE"
